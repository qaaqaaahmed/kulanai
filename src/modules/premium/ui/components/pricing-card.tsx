...
const pricingCardIconVariants = cva("size-5", {
  variants: {
    variant: {
      ...
      highlighted: "fill-black text-white",
    },
  },
...
const pricingCardTextSecondaryVariants = cva("text-neutral-700", {
  variants: {
    variant: {
    ...
    <div className={cn(pricingCardVariants({ variant }), className, "border")}>
      <div className="flex items-end justify-between gap-x-4">
        <div className="flex flex-col gap-y-2">
          ...
          <p
            className={cn(
              pricingCardTextSecondaryVariants({ variant }),
              "text-xs",
            )}
          >
            {description}
        ...
        <div className="flex items-end shrink-0 gap-x-0.5">
          ...
          <span className={pricingCardTextSecondaryVariants({ variant })}>
            {priceSuffix}
          </span>
        </div>
      </div>
